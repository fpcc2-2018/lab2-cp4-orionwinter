...
Como podemos ver, a taxa geral de clicks diária é razoavelmente baixa, visto que pode variar entre 37% e 41%, além de não existir diferença significativa, com 95% de confiança, da proporção de pesquisas que geraram click entre os dias. Ou seja, a maioria das sessões de busca, não resulta em nenhum click.
...
Vemos que existe uma diferença estatisticamente relevante, com 95% de confiança, da taxa geral de clicks diária entre os grupos, visto que o intervalo de confiança do grupo A é sempre axima do grupo B, independente do dia.

Com isso concluímos que a taxa de click geral diária pode ficar entre 37% e 41%, mas quando variamos entre os grupos, percebemos que a taxa do grupo A fica entre cerca de 63% e 69%, enquanto a taxa do grupo B pode ficar entre 12% e 21%. Logo existe uma diferença significativa entre o grupo A e o grupo B, com 95% de confiança. Além do mais, o grupo A tem a taxa de click geral diária mais de três vezes maior que o grupo B, independente do dia.
...
Vamos agora tentar inferir quais resultados as pessoas tendem a clicar primeiro baseado na amostra dos dados, aplicando bootstrap na amostra e medindo a média do índice do primeiro click, com 95% de confiança:
...
Podemos verificar que com 95% de confiança, a média do índice da posição média do primeiro click na página está entre 3.28 e 4.42, ou seja, a posição média fica entre o elemento 3 e o 5 da página.
...
Com isso, concluímos que no geral, a posição média do click se encontra entre o terceiro e o quinto elemento da página, com 95% de confiança. Ao analisar dia a dia, excluindo o dia 01/03 e 04/03, a posição média não varia tanto, visto que a posição média para o primeiro click se encontra até o quarto elemento da página, com 95% de confiança. Vale ressaltar que utilizamos a média para verificar a tendência central, medida esta que é sensível à outliers.
