...
Vemos que os dados vão do dia 01/03/2016 à 08/03/2016, onde os finais de semana tem menos observações que os demais. Como o dia 08/03 foi até as 20h, tem menos observações que os outros dias de semana.
...
Vemos que 75% das buscas retornam até 20 resultados, porém existem alguns que retornam bem mais que este valor. 
...
Vemos que a concentração maior de clicks está próxima de zero, onde 75% das vezes não existe nenhum click, porém a distribuição tem uma cauda longa a direita, tendo um caso que o usuário clicou mais de 30 vezes nos itens da busca.
...
Vemos que a maior parte dos indices tem valor baixo, mas existem alguns com valor acima de 4000, o que é uma anomalia, pois vimos anteriormente que as buscas só retornam até cerca de 500 resultados.

Visto isso, vamos remover as buscas que tiverem o índice do primeiro click acima do total de resultados.

```{r warning=FALSE, message=FALSE}
buscas.filtradas <- buscas.filtradas %>% 
...
Como a sessão não tem nenhuma restrição, não faremos nenhum filtro com a mesma.
...
Para responder esta pergunta veremos primeiramente a distribuição dos valores do índice do primeiro click, para isso, vamos filtrar as observações que não informam onde clicaram:
...
Vemos que a maior concentração está em índices baixos, abaixo de 10, como podemos ver no histograma a seguir:
...
buscas.com.click %>% 
    ...
    ggplot(aes(x = first_click)) +
        geom_histogram(binwidth = 1) +
        facet_grid(dia.do.ano ~ .) +
        labs(x = "índice do primeiro click", y = "contagem") +
        theme_bw()
```

As distribuições dos clicks parecem seguir a mesma distribuição, independente do dia da semana, como podemos confirmar com as médias, medianas e proporção de clicks no primeiro link, visto abaixo.
...
Podemos ver que existem muitos pontos concentrados no canto esquerdo variando bastante no número de clicks, visto que é mais comum ter sessões curtas, como também existe alguns pontos quase paralelos ao eixo horizontal, formando um "L".

Desta forma, concluímos que não parece existir relação significativa entre as variáveis da duração da sessão e do número de clicks, como podemos ver abaixo pela correlação de pearson:
